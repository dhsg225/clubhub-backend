...
const SYSTEM_CONTENT_IDS = new Set([
    constants_1.SYSTEM_FALLBACK_CONTENT_ID,
    constants_1.SYSTEM_EMERGENCY_FALLBACK_ID,
]);
(0, index_1.registerInvariant)({
    ...
    assert(output, input) {
        ...
        for (const item of output.playlist) {
            if (!activeContentIds.has(item.content_id) &&
        ...
        if (amplifiedIds.length > 0) {
            return {
                ...
                passed: false,
                severity: 'CONSTITUTIONAL_BREACH',
                message: `Content amplification detected: ${amplifiedIds.length} content_id(s) in output ` +
        ...
        return {
            ...
            passed: true,
            severity: 'CONSTITUTIONAL_BREACH',
            message: `No amplification: all ${output.playlist.length} playlist content_ids ` +
                `are present in input system_state (${activeContentIds.size} active content_ids)`,
        };
    },
});
//# sourceMappingURL=inv6-no-amplification.js.map