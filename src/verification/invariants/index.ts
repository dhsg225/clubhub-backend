...
var _registryStoreStore: Map<string, InvariantDefinition> | undefined;

function getRegistry(): Map<string, InvariantDefinition> {
  if (_registryStoreStore === undefined) {
    _registryStoreStore = new Map<string, InvariantDefinition>();
  }
 ...
 */
export function registerInvariant(def: InvariantDefinition): void {
  const registry = getRegistry();
  if (registry.has(def.id)) {
    throw new Error(
      `Duplicate invariant registration: "${def.id}". ` +
 ...
 */
export function registeredInvariantIds(): string[] {
 ...
 */
export function assertAllInvariantsRegistered(): void {
  const required = [
  ...
  if (missing.length > 0) {
    throw new Error(
      `CONSTITUTIONAL_BREACH: Missing invariant registrations: ${missing.join(', ')}. ` +
