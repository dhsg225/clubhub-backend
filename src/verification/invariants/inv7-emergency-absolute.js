...
(0, index_1.registerInvariant)({
    ...
    assert(output, input) {
        ...
        if (output.resolution_level !== types_1.RESOLUTION_LEVELS.LEVEL_0_EMERGENCY) {
            return {
                invariantId: 'INV-7',
                passed: false,
                severity: 'CATASTROPHIC',
                message: `CATASTROPHIC: Active emergency (id="${emergency.id}") exists but PRE resolved at ` +
        ...
        if (output.playlist.length === 0) {
            return {
                ...
                passed: false,
                severity: 'CATASTROPHIC',
                message: `CATASTROPHIC: Active emergency (id="${emergency.id}") resolved at LEVEL_0 ` +
        ...
        const hasEmergencyContent = output.playlist.some(item => item.content_id === emergency.content_id);
        ...
        const nonEmergencyItems = output.playlist.filter(item => item.content_id !== emergency.content_id);
        if (nonEmergencyItems.length > 0) {
            return {
                invariantId: 'INV-7',
                passed: false,
                severity: 'CATASTROPHIC',
                message: `CATASTROPHIC: Active emergency playlist contains ${nonEmergencyItems.length} ` +
        ...
        return {
            ...
            passed: true,
            severity: 'CATASTROPHIC',
            message: `Emergency absoluteness holds: LEVEL_0 resolution, ` +
                `emergency content "${emergency.content_id}" is sole playlist item`,
        };
    },
});
//# sourceMappingURL=inv7-emergency-absolute.js.map