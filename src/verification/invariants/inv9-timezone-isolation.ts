...
import { assertIanaTimezone } from '../../pre/algorithms/venue-local-time';
  ...
  assert(_output, input) {
    ...
    if (typeof timezone !== 'string' || timezone.length === 0) {
      return {
        ...
        severity: 'CONSTITUTIONAL_BREACH',
        message:
          `Timezone isolation violation: venue.timezone is "${timezone}". ` +
    ...
    try {
      ...
      return {
        ...
        severity: 'CONSTITUTIONAL_BREACH',
        message:
          `Timezone isolation violation: venue.timezone="${timezone}" failed IANA validation. ` +
          `Error: ${String(err)}`,
    ...
    for (const pattern of UTC_OFFSET_PATTERNS) {
      if (pattern.test(timezone)) {
        return {
          ...
          severity: 'CONSTITUTIONAL_BREACH',
          message:
            `Timezone isolation violation: venue.timezone="${timezone}" matches a ` +
