...
import { SOV_WARNING_THRESHOLD } from '../../pre/constants';
...
registerInvariant({
  ...
  assert(output, _input) {
    ...
    if (output.playlist.length > 0) {
      ...
      if (diff > 0.01) {
        return {
          ...
          severity: 'CONSTITUTIONAL_BREACH',
          message:
            `Sponsor fraction mismatch: content_mix.sponsor_pct=${storedSponsorPct} ` +
    ...
    if (mix.sponsor_pct > SOV_WARNING_THRESHOLD) {
      if (!sponsorTrace || !sponsorTrace.sov_warning_active) {
        return {
          ...
          severity: 'CONSTITUTIONAL_BREACH',
          message:
            `SOV penetration violation: sponsor_pct=${mix.sponsor_pct} exceeds ` +
            `SOV_WARNING_THRESHOLD=${SOV_WARNING_THRESHOLD} but sov_warning_active is ` +
          ...
          detail: {
            ...
            threshold: SOV_WARNING_THRESHOLD,
    ...
    for (const item of output.playlist) {
      if (typeof item.sponsored !== 'boolean') {
        return {
          ...
          severity: 'CONSTITUTIONAL_BREACH',
          message:
            `Playlist item "${item.content_id}" has sponsored=${item.sponsored} (not boolean). ` +
    ...
    if (output.resolution_level !== 0 && Math.abs(mixSum - 1.0) > 0.01) {
      return {
        ...
        severity: 'CONSTITUTIONAL_BREACH',
        message:
          `content_mix percentages sum to ${mixSum.toFixed(4)}, expected 1.0000. ` +
    ...
    return {
      ...
      severity: 'CONSTITUTIONAL_BREACH',
      message:
        `Sponsorship non-penetration holds: sponsor_pct=${mix.sponsor_pct}, ` +
