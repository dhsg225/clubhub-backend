    ...
    assert(output, _input) {
        ...
        if (output.playlist.length === 0) {
            return {
                ...
                passed: false,
                severity: 'CONSTITUTIONAL_BREACH',
                message: 'output.playlist is empty. PRE must always produce at least one item. ' +
                    `When no content source is available, PRE returns "${constants_1.SYSTEM_FALLBACK_CONTENT_ID}".`,
            };
        }
        ...
        if (!Number.isInteger(output.resolution_level) ||
...
//# sourceMappingURL=inv2-totality.js.map