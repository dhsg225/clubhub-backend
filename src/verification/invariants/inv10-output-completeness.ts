...
const REQUIRED_OUTPUT_FIELDS: ReadonlyArray<string> = [
  'screen_id',
  'resolved_at',
  'resolution_level',
  'is_fallback',
...
const REQUIRED_CONTENT_MIX_FIELDS: ReadonlyArray<string> = [
  'campaign_pct',
  'sponsor_pct',
  'override_pct',
  'fallback_pct',
...
const REQUIRED_REASON_TRACE_FIELDS: ReadonlyArray<string> = [
  'level_0_emergency',
  'level_1_operational',
  'level_2_scheduled',
  'level_3_campaign',
...
const REQUIRED_PLAYLIST_ITEM_FIELDS: ReadonlyArray<string> = [
  'content_id',
  'duration_ms',
  'weight',
  'source',
...
registerInvariant({
  ...
  assert(output, _input) {
    ...
    // 1. Top-level fields
    const missingTopLevel = REQUIRED_OUTPUT_FIELDS.filter(
      field => !(field in out)
    );
    ...
    const mix = out['content_mix'] as Record<string, unknown>;
    ...
    const trace = out['reason_trace'] as Record<string, unknown>;
    ...
    if (Array.isArray(playlist)) {
      for (let i = 0; i < playlist.length; i++) {
        const item = playlist[i] as Record<string, unknown>;
