...
import type { PRE_Input, ScheduleRecord, ContentItemRecord, PlaylistItem, OverrideRecord } from '../types';
import { LEVEL_3_CAMPAIGN, SYSTEM_FALLBACK_CONTENT_ID, SYSTEM_FALLBACK_DURATION_MS } from '../constants';
import { weightedPlaylistResolver } from '../algorithms/swrr';
import { toVenueLocal } from '../algorithms/venue-local-time';
 ...
 */
function expandCampaignSchedule(
  schedule: ScheduleRecord,
  contentItems: ContentItemRecord[],
  overrideContentIds: ReadonlySet<string>
): ScheduleRecord[] {
  return contentItems
...
function deduplicateByContentId(schedules: ScheduleRecord[]): ScheduleRecord[] {
  const best = new Map<string, { schedule: ScheduleRecord }>();
  ...
  for (const schedule of schedules) {
    ...
    if (schedule.specificity > ex.specificity) { best.set(contentId, { schedule }); continue; }
    if (schedule.specificity < ex.specificity) { continue; }
    // Rule 2: higher priority wins
    if (schedule.priority > ex.priority) { best.set(contentId, { schedule }); continue; }
    if (schedule.priority < ex.priority) { continue; }
    // Rule 3: lexicographically smaller id wins
    if (schedule.id < ex.id) { best.set(contentId, { schedule }); }
  }

  return Array.from(best.values()).map((d) => d.schedule);
}

export function resolveLevel3(
  input: PRE_Input,
  ...
  const overrideContentIds = new Set<string>(
    input.system_state.overrides
      .map((o: OverrideRecord) => o.content_id)
      .filter((id): id is string => id !== null)
  );

  // Expand campaign-linked schedules (content_id=null) to concrete content_id entries
  const expanded: ScheduleRecord[] = [];
  ...
  if (deduped.length === 0) {
    return {
      playlist: [],
      terminatingLevel: LEVEL_3_CAMPAIGN,
  ...
  const playlistItems: PlaylistItem[] = deduped.map((schedule) => {
    const contentItem: ContentItemRecord | undefined = input.system_state.content_items.find(
      (c) => c.id === schedule.content_id
    );
    const contentId = contentItem ? (schedule.content_id as string) : SYSTEM_FALLBACK_CONTENT_ID;
    ...
    return {
      ...
      source:      LEVEL_3_CAMPAIGN,
      sponsored:   false,
    };
  });

  const ordered = weightedPlaylistResolver(playlistItems);
  ...
  if (winning.days_of_week.length > 0) {
    const venueLocal = toVenueLocal(input.at, input.system_state.venue.timezone);
