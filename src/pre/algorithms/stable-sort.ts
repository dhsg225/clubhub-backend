 ...
 */
export function deterministicSortByNumericKey<T extends { id: string }>(
  items: T[],
 ...
 */
export function deterministicSortByStringKey<T extends { id: string }>(
  items: T[],
 ...
 */
export function deterministicSortByFields<T extends { id: string }>(
  items: T[],
 ...
 */
export function deterministicSortStrings(strings: string[]): string[] {
 ...
 */
export function deterministicSortPacketIds(packetIds: string[]): string[] {
 ...
 */
export function assertTotalOrdering<T>(
  items: T[],
  ...
  for (let i = 0; i < items.length; i++) {
    for (let j = i + 1; j < items.length; j++) {
      const result = comparator(items[i] as T, items[j] as T);
      if (result === 0) {
        throw new Error(
          `assertTotalOrdering: non-total ordering detected for "${label}". ` +
