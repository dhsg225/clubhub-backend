 ...
 */
function encodeToUtf8Bytes(str: string): Uint8Array {
 ...
 */
export function fnv1a32Bytes(bytes: Buffer | Uint8Array): string {
  let hash = FNV1A_OFFSET_BASIS;
 ...
 */
export const FNV1A_TEST_VECTORS = [
 ...
 */
export function verifyFnv1a32Implementation(): void {
  for (const vector of FNV1A_TEST_VECTORS) {
    const actual = fnv1a32(vector.input);
    if (actual !== vector.expected) {
      throw new Error(
        `FNV-1a 32-bit implementation non-conformant: ` +
