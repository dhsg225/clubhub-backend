...
export interface ContentMix {
  campaign_pct:  number;  // 4 decimal places
  sponsor_pct:   number;  // 4 decimal places
  override_pct:  number;  // 4 decimal places
  fallback_pct:  number;  // 4 decimal places
  system_pct:    number;  // 4 decimal places
}
...
export interface OverrideRecord {
  ...
  is_operational: boolean;      // true = LEVEL_1; false = LEVEL_2
  priority:      number;
  reason:        string | null;
...
export interface ScheduleRecord {
  ...
  content_id:          string | null; // direct content (non-campaign schedule)
  target_type:         'screen' | 'tv_group' | 'area' | 'venue';
  target_id:           string;
  specificity:         number;
  starts_at:           number;        // UTC ms
...
export interface SponsorshipContractRecord {
  ...
  area_id:       string;
  content_id:    string;
  sov_pct:       number;   // 4 decimal places; share-of-voice percentage [0,1]
  starts_at:     number;   // UTC ms
...
export interface ScreenDeliveryLogRecord {
  ...
  checksum:      string;  // FNV-1a 32-bit hex
  resolution_level: ResolutionLevel;
}
